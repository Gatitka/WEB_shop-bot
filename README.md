...
# Рассылка
кнопка "разослать всем" показывается до того как рассылка была запущена хотя бы 1 раз.


# Привязка телеграм аккаунта
если у пользователя нет привязанного аккаунта, то проходится логин в телеграме от лица бота, дающий разрешение
боту писать пользователю. Создается Messenger Account.
Данные телеграма присылаются на бэкэнд и проходят валидацию по hash. Ключи+значения перекодируются и сравниваются с токеном бота.
Если какой-то ключ/значение не пришли или пришли иные, то будет ошибка "Bad signature".

При смене города пользователю показывается новая кнопка привязки Tm аккаунта.
Если не пройти логин в новом боте и сохранить будет ошибка.

При добавлении нового Tm аккаунта шлется тестовое сообщение, т.к. бэк не знает, дал ли пользователь боту право писать или нет.
При смене города и не смотря на успешную верификацию мы опять отправляем тестовое сообщение, чтобы убедиться, что бот может писать ползователю.

Про успешной подписке пользователю отправляется сообщение об этом.

Если Tm аккаунта нет в бд, то просто создается новый и привязывается к существующему base_profile.
Если Tm аккаунт уже есть в бд (заходил до этого через бота и имеет аккаунты-заглушки и не привязан ни к какому профилю), то MA отвязывается от base_profile и привязывается к нужному base_profile. Registered ставится True, подписка ставится как у нового base_profile.
Заказы MA, полученные через бота (source='3'), переписываются на новый base_profile.

Если Тм аккаунт уже привязан к какому-то действующему пользователю, registered True, то будет ошибка.

Если пользователь отвязывает аккаунт, то передается None. связь удаляется, но все аккаунты остаются.
ma.registered = False, ma.subscription  = False. заказы не меняются и остаются user=bp, ma=ma.


# Managment_commands

python manage.py test_adaptation    - перепишет админ чаты на значения из .env (предварительно нужно поставить тестовые в .env)
